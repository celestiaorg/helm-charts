...
toml-node: prompt_node_version
	...
	./$(CONVERT_TOML_TO_TEMPLATE_SCRIPT) $(CONFIG_DIR)/node/$(CELESTIA_NODE_VERSION)/config.toml node
...
yaml-node: prompt_node_version
	...
	./$(CONVERT_TOML_TO_YAML_SCRIPT) $(CONFIG_DIR)/node/$(CELESTIA_NODE_VERSION)/config.toml
...
.PHONY: app
app: extract-app toml-app yaml-app

# Group targets for celestia-node
.PHONY: node
node: extract-node toml-node yaml-node
...
help:
	...
	@echo "  make toml-app              Convert TOML to templated TOML for celestia-app"
	@echo "  make yaml-app              Convert TOML to YAML for celestia-app"
	@echo "  make extract-node          Extract config files for celestia-node"
	@echo "  make toml-node             Convert TOML to templated TOML for celestia-node"
